...
def add_summary_value(writer, key, value, iteration):
    summary = torch.tensor([value]).float().reshape(1, 1)
    # summary = tf.Summary(value=[tf.Summary.Value(tag=key, simple_value=value)])
    writer.add_scalar(key, summary, iteration) 
...
def train(epoch, opt):
    ...
    for step in range(len(dataloader)-1):
        ...
        input_imgs= Variable(img.cuda())
        input_seqs = Variable(iseq.cuda())
        gt_seqs = Variable(gts_seq.cuda())
        input_num = Variable(num.cuda())
        input_ppls = Variable(proposals.cuda())
        gt_bboxs = Variable(bboxs.cuda())
        mask_bboxs = Variable(box_mask.cuda())
        # with torch.no_grad(): 
...
if __name__ == '__main__':
    ...
    dataloader_val = torch.utils.data.DataLoader(dataset_val, batch_size=opt.batch_size,
                                            shuffle=False, num_workers=opt.num_workers)
    dataset_test = DataLoader(opt, split='test')
    ...
    tf_summary_writer = SummaryWriter(opt.checkpoint_path)
    infos = {}
    histories = {}
    #########################################load because interupted###################################
    model.load_state_dict(torch.load('/root/NeuralBabyTalk/save/model-best.pth'))
...
#######################################################################################################################
        ...
        if epoch % opt.val_every_epoch == 0:
            lang_stats, predictions = eval(opt,dataloader_val)
            ...
            if best_flag:
                ...
                with open(os.path.join(opt.checkpoint_path, 'infos_'+opt.id+'-best.pkl'), 'wb') as f:
                    pickle.dump(infos, f)
                with open(os.path.join(opt.checkpoint_path, 'preds_val'+opt.id+'.pkl'), 'wb') as f:
                    pickle.dump(predictions, f)
    opt.beam_size = 3
    opt.val_split = 'test'
    lang_stats, predictions = eval(opt, dataloader_test)
    with open(os.path.join(opt.checkpoint_path, 'preds_test'+opt.id+'.pkl'), 'wb') as f:
        pickle.dump(predictions, f)